...
public interface ClubManagementCommandService {
     ...
     * @return 생성된 독서모임 ID
     */
    Long createClub(Long memberId, ClubRequestDTO.ClubDetailDTO request);
     ...
     * @param memberId 운영진 ID -> 운영진인지 확인하는 로직 필요 ClubMember에서 Role 확인 -> 어노테이션으�� 처리 고려
     * @param request 모임 수정 요청 DTO
     * @return 생성된 독서모임 ID
     */
    Long updateClub(Long clubId, Long memberId, ClubRequestDTO.ClubDetailDTO request);
}

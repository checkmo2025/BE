...
public class OAuth2CallbackExceptionFilter extends OncePerRequestFilter {
    ...
    private void handleUnexpectedException(
        ...
        RuntimeException diagnosticException = diagnosticException(traceId, request, exception, rootCause);

        log.error(
                "OAuth2 콜백 처리 중 예상하지 못한 예외: traceId={}, provider={}, type={}, rootType={}, uri={}, topFrame={}",
                traceId,
                provider(request),
        ...
        sentryCaptureClient.captureException(diagnosticException);
    ...
    private RuntimeException diagnosticException(
            String traceId,
            HttpServletRequest request,
            Exception exception,
            Throwable rootCause
    ) {
        RuntimeException diagnosticException = new RuntimeException(
                "OAuth2 callback failed: traceId=" + traceId
                        + ", provider=" + provider(request)
                        + ", type=" + exception.getClass().getSimpleName()
                        + ", rootType=" + rootCause.getClass().getSimpleName()
        );
        diagnosticException.setStackTrace(rootCause.getStackTrace());
    ...
    private String topFrame(Throwable exception) {
